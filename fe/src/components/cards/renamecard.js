...
import { setRename } from '../../states/cardModalSlice';
...
function RenameCard(props) {
  ...
  const isModalOpen = useSelector(state => state.cardModal.value.rename)
  ...
  const EMPTYDIC = {rows:[{}], type:'rename', datatypes:datatypes}
  ...
  const [dic, setDic] = useState(EMPTYDIC);
    ...
    const handleOk = () => {
      ...
      axios.post(`${process.env.REACT_APP_BACKEND_URL}/rename`, {data:currentData,datatypes:datatypes,dic:dic})
      .then(res => {
  ...
  const handleOkEdit = () => {
    ...
    return (
        ...
        <Card className='TransformCard'>
            ...
            />
            <br/>
            <p className='cardTest'>Rename</p>
        </Card>
        </div>
        { !isEdit ? 
        <RenameModal isModalOpen={isModalOpen} handleCancel={handleCancel} handleChange={handleChange} handleOk={handleOk} error={error} dic={dic}  delRow={delRow} addRow={addRow} changeRows={changeRows}/>
        : 
        <RenameModal isModalOpen={isModalOpen} handleCancel={handleCancel} handleChange={handleChangeEdit} handleOk={handleOkEdit} error={error} dic={oldDic} delRow={delRowEdit} addRow={addRowEdit} changeRows={changeRowsEdit}/>
        }
