...
import { setDatatype } from '../../states/cardModalSlice';
...
function DatatypeCard(props) {
  ...
  const isModalOpen = useSelector(state => state.cardModal.value.datatype)
  ...
  const [dic, setDic] = useState(EMPTYDIC);
    ...
    const handleOk = () => {
      ...
      axios.post(`${process.env.REACT_APP_BACKEND_URL}/datatype`, {data:currentData,datatypes:datatypes,dic:dic})
      .then(res => {
  ...
  const handleOkEdit = () => {
    ...
    return (
        ...
        <Card className='TransformCard'>
            ...
            />
            <br/>
            <p className='cardTest'>Datatype</p>
        </Card>
        </div>
        { !isEdit ? 
        <DatatypeModal isModalOpen={isModalOpen} handleCancel={handleCancel} handleChange={handleChange} handleOk={handleOk} error={error} dic={dic}  delRow={delRow} addRow={addRow} changeRows={changeRows}/>
        : 
        <DatatypeModal isModalOpen={isModalOpen} handleCancel={handleCancel} handleChange={handleChangeEdit} handleOk={handleOkEdit} error={error} dic={oldDic} delRow={delRowEdit} addRow={addRowEdit} changeRows={changeRowsEdit}/>
        }
