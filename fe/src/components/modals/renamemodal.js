...
function RenameModal(props) {
  ...
  console.log(props.dic)
    ...
    return (
        <div>
          <Modal title="Rename Transformation" open={props.isModalOpen} onOk={props.handleOk} onCancel={props.handleCancel}>
          {props.error==='' ? null : <Alert message={props.error} type="error" />}       
            {props.dic.rows !== undefined ? props.dic.rows.map((element,index) => (
              <div>
                ...
                <Input onChange={e => props.changeRows(e.target.value, index, 'new_name')}
                  value={element.new_name} /><br/>
                <hr/>
                </div>
            )) : null}
